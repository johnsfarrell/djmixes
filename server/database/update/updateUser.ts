...
async function createUser(
  ...
  try {
    const [existingUsers] = await connection.query('SELECT * FROM users WHERE username = ? OR email = ?', [
      username,
      email
    ]);
    ...
    console.log(`User created with ID: ${(result as ResultSetHeader).insertId}`);
